...
from src.cultiva_lab.exceptions import (
    UserNotFoundError, AuthorizationError, InvalidInputError,
    UserAlreadyExistsError, AdminAlreadyExistsError
)

from ..schemas.auth import (
    LoginRequest, RegisterRequest, RegisterAdminRequest,
    LoginResponse, UserResponse
)
from ..dependencies import (
    get_user_service, create_access_token, get_current_user
)
...
def register_admin(request: RegisterAdminRequest, user_service=Depends(get_user_service)):
    try:
        user = user_service.register_admin(request.admin_key, request.username, request.password)
        return UserResponse(id=user.id, username=user.username, role=user.role.value)
